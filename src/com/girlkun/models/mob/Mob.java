...
public class Mob {
    ...
    public static void initMobMiNuong(Mob mob, byte level) {
        mob.point.dame = level * 3250 * mob.level * 4;
        mob.point.maxHp = level * 12472 * mob.level * 2 + level * 7263 * mob.tempId;
    }
        ...
        try {
            if ((!player.isPet && player.getSession().actived && player.setClothes.setDHD == 5) || (player.isPet && ((Pet) player).master.getSession().actived && ((Pet) player).setClothes.setDHD == 5)) {
                ...
                if (Util.isTrue(100, 100)) {
                    Item thucan = ItemService.gI().createNewItem(Manager.thucan[(Util.nextInt(0,4))]);
                     Service.getInstance().sendThongBao(player, "You received item " + thucan.template.name);
                                        InventoryServiceNew.gI().addItemBag(player, thucan);
                InventoryServiceNew.gI().sendItemBags(player);
              }
